...
import { David_Libre, Chivo } from 'next/font/google';
...
export const metadata: Metadata = {
  title: 'Roamr',
  description: 'Explore the world with Roamr',
};
...
export default function RootLayout({
  ...
  return (
    <html lang="en" className="bg-background">
      <head />
      <body className={`${davidLibre.variable} ${chivo.variable} antialiased`}>{children}</body>
    </html>
  );
}

function TWBreakpointLogger() {
  return (
    <>
      <div className="fixed right-0 bottom-0 m-2 flex items-center rounded border border-gray-400 bg-gray-300 p-2 text-sm text-pink-600">
        <svg
